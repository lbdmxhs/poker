...
var DEFAULT_EXPIRE =  60 * 1000;	// 一分钟
...
pro.auth = function(token, cb) {
	var self = this;
	var res = tokenService.parse(token, this.secret,this.iv);
	if(!res) {
	...
	userDao.getUserById(res.uid, function(err, user) {
		...
		token = tokenService.create(user.id, Date.now(), self.secret,self.iv);
		cb(null, Code.OK, user,token);
	});
};
